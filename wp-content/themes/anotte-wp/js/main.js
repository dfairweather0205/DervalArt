...
//------------------------------------------------------------------------
    ...
    $('.more-product-categoies').on('click', function () {
        if (!$(this).hasClass('click-off')) {
            ...
            count++;
            loadProductCategoryItems(count, currentProductCategories,type,term_id);
        }
    });

    function loadProductCategoryItems(pageNumber,currentProductCategories,type,term_id) {
        switch(type) {
            case 'category': 
                $.ajax({
                    url: ajax_var.url,
