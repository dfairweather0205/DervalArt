...
if ( ! class_exists( __NAMESPACE__ . '\CustomGallery' ) ) {
	...
	class CustomGallery {
		...
		public function generateGalleryHtml( $id, array $imageIds ) {
			...
			return sprintf(
				$html,
				esc_html__( 'Open', 'pikart-base' ),
				esc_html__( 'Reset', 'pikart-base' ),
				$id,
				$id,
				implode( ',', $imageIds ),
		...
		private function buildMetaFields() {
			return $this->metaConfigBuilder
				...
				->label( esc_html__( 'Video URL', 'pikart-base' ) )
				->description( esc_html__( 'Enter Youtube or Vimeo link', 'pikart-base' ) )
				->build();
		}
