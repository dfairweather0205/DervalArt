	...
	class GoogleFontsHelper {
		...
		private function registerAddGoogleFontAjax() {
			...
			add_action( 'wp_ajax_' . PIKART_SLUG . '_add_google_font', function () use (
				...
				$errorMessage      = esc_html__( 'Invalid font. Retype it carefully again.', 'pikart-base' );
				...
				if ( in_array( $googleFont, $fonts ) ) {
					wp_send_json_error(
						esc_html__( 'Font already exists. See font family dropdown-lists', 'pikart-base' ) );
				...
				if ( strpos( $googleFont, '|' ) !== false ) {
					wp_send_json_error( esc_html__( 'Please add only one font at once', 'pikart-base' ) );
				...
				wp_send_json_success( array(
					'message'     => esc_html__( 'Font was added. See font family dropdown-lists', 'pikart-base' ),
